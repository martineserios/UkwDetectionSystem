...
import pytest
from dataset_partition import (ELEMENTS, SET_TYPES, _build_dataset_structure,
                               _build_dataset_structure_on_disk,
                               _get_dataset_by_named_parts_ratios,
                               _get_dataset_paths_by_elements,
                               _get_images_labels, _get_sublists_from_n_parts,
                               _get_sublists_from_named_parts,
                               _get_sublists_from_ratios,
                               _turn_parts_into_ratios, _write_files,
                               partition_dataset)

# Define dummy paths and data
DUMMY_DATASET_PATH = "dummy_dataset"
DUMMY_NEW_DATASET_PATH = "new_dummy_dataset"

@pytest.fixture
def setup_dummy_dataset():
    # Create a dummy dataset structure
    if not os.path.exists(DUMMY_DATASET_PATH):
        os.mkdir(DUMMY_DATASET_PATH)
    for set_type in SET_TYPES:
        set_type_path = os.path.join(DUMMY_DATASET_PATH, set_type)
        os.mkdir(set_type_path)
        for element in ELEMENTS:
            element_path = os.path.join(set_type_path, element)
            os.mkdir(element_path)
            # Create dummy files
            for i in range(10):
                with open(os.path.join(element_path, f"{element}_{i}.txt"), 'w') as f:
                    f.write(f"{element} content {i}")
    ...
    if os.path.exists(DUMMY_NEW_DATASET_PATH):
        shutil.rmtree(DUMMY_NEW_DATASET_PATH)

def test_build_dataset_structure():
    structure = _build_dataset_structure(3, SET_TYPES, ELEMENTS)
...
def test_get_sublists_from_named_parts():
    ...
    named_sublists = _get_sublists_from_named_parts(original_list, named_parts)
...
def test_get_dataset_by_named_parts_ratios():
    ...
    named_parts = ['part1', 'part2', 'part3']
    ratios = [0.2, 0.3, 0.5]
    dataset = _get_dataset_by_named_parts_ratios(original_list, named_parts, ratios)
    assert len(dataset) == 3

def test_write_files(setup_dummy_dataset):
    files_to_write = [os.path.join(DUMMY_DATASET_PATH, 'train', 'images', f'images_{i}.txt') for i in range(5)]
    _write_files(DUMMY_NEW_DATASET_PATH, '1', 'train', 'images', files_to_write)
    for i in range(5):
        assert os.path.exists(os.path.join(DUMMY_NEW_DATASET_PATH, '1', 'train', 'images', f'images_{i}.txt'))
...
######################
# Improved by chat gpt, but a bit slower. SHOULD CHECK
