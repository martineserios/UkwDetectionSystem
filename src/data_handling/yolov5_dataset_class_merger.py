# ref: https://github.com/edybk/yolov5-dataset-class-merger
...
def create_index_mapping(original_names, name_mapping, classes_to_delete):
    print(f"original names: {original_names}")
    original_mapping = create_mapping_from_names(original_names)
    print(f"original mapping: {original_mapping}")
    print(f"name mapping: {name_mapping}")
    print(f"classes to delete: {classes_to_delete}")
    
    # Remove classes to delete from name_mapping
    name_mapping = {k: v for k, v in name_mapping.items() if k not in classes_to_delete}
    
    new_names = sorted(list(set([name for name in name_mapping.values()])))
    print(f"new names: {new_names}")
    new_mapping = create_mapping_from_names(new_names)
    print(f"new mapping: {new_mapping}")
    old_index_to_new_index = dict([(original_mapping[old_name], new_mapping[new_name]) for old_name, new_name in name_mapping.items() if old_name not in classes_to_delete])
    print(f"old index to new index: {old_index_to_new_index}")
    return new_names, old_index_to_new_index
...
def convert_label(old_path, new_dir, index_mapping):
    ...
    if new_lines:  # Only create a new file if there are any labels left
        label_name = old_path.split(os.path.sep)[-1]
        new_path = os.path.join(new_dir, label_name)
        with open(new_path, 'w') as f:
            for index, line in enumerate(new_lines):
...
def convert(root, new_root, original_names, name_mapping, classes_to_delete):
    started = time.time()
    if os.path.isdir(new_root):
        print(f"failure: new root already exists!")
        return
    ...
    new_names, index_mapping = create_index_mapping(original_names, name_mapping, classes_to_delete)
